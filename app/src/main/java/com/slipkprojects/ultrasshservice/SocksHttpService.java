...
implements SkStatus.StateListener
{
	...
	public synchronized void startTunnel() {
		...
		SkStatus.logInfo(String.format("Your IP Address: %s", getIpPublic()));
		...
		try {
			...
			SkStatus.logInfo("Starting Service");
	...
	private void showNotification(final String msg, final String channel,
		...
		if(tunnelType==Settings.bTUNNEL_TYPE_SSH_PROXY){
			...
			type = "UDP Protocol (Hysteria) | HTTP Injector";
			...
			type = "Secure Shell (Vmess/Vless) | HTTP Injector";
		} else{
		...
		if (status == ConnectionStatus.LEVEL_WAITING_FOR_USER_INPUT) {
            PendingIntent pIntent = PendingIntent.getActivity(this, 0, intent, getFlags());
	...
	public static PendingIntent getGraphPendingIntent(Context context) {
		...
		intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
		return PendingIntent.getActivity(context, 0, intent, getFlags());
	}
	...
	public static int getFlags() {
		return Build.VERSION.SDK_INT >= 31 ? PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT : 0;
	}
}